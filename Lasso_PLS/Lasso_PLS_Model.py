...
'''
def splitDataSet(x, y,k):
    m =shape(x)[0]
    train_sum = int(round(m * k))
...
#PLS回归系数压缩
def PLS_xish(e0, f0, ntest, th_k,lambd_k):
...
def canshu1():
    return y0
def canshu2():
    y_predict = x0 * xish1 + tile(ch0[0, :], (row, 1))
...
# 随机划分数据集- 7:3
#train_x, train_y, test_x, test_y = splitDataSet(x0, y0, k )
#print train_x,'\n','*****','\n', train_y,'\n','*****','\n', test_x, '\n','*****','\n',test_y
#标准化
...
xishu = w_star * beta
#反标准化
ch0, xish1 = Calxishu(xishu, mean_x, mean_y, std_x, std_y)
...
#     plt.annotate('y_predict', xy=(6, 0.090), xytext=(4, 0.15),
